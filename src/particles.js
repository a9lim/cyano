...
const Particles = {
    ...
    draw(ctx, state) {
        ...
        // ── Photons (flat translucent circles, no glow) ──
        if (!_photonFill) {
            _photonFill = _r(EnzymeStyles.roleColors.photon.stroke, 0.75);
        }
        for (let i = this.photons.length - 1; i >= 0; i--) {
            ...
            ctx.beginPath();
            ctx.arc(ph.x + (ph.tx - ph.x) * t, ph.y + (ph.ty - ph.y) * t, 3.5, 0, _TWO_PI);
